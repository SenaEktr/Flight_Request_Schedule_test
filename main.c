...
typedef struct od_demand
{
    ...
    // char *ST;              // Start Time - for mutations based on delays
...
UATFM_Route_Network_Edge UATFM_route_network[NUMBER_OF_EDGES + 1];
OD_Demand *demands;
unsigned short all_alternative_paths_for_ODs[NUMBER_OF_ODS][NUMBER_OF_ALTERNATIVES + 1][MAX_NUMBER_OF_EDGES_IN_ROUTE] = {0};
unsigned short alternative_paths_for_ODs[NUMBER_OF_ODS * MULTIPLICITY_STRING_REP][NUMBER_OF_ALTERNATIVES][MAX_NUMBER_OF_EDGES_IN_ROUTE] = {0};

int number_of_demand;
int edge_matrix[POPULATION_SIZE][NUMBER_OF_TIME_INTERVALS][NUMBER_OF_EDGES][NUMBER_OF_LINES] = {0};
int edge_matrix_for_individual1[NUMBER_OF_TIME_INTERVALS][NUMBER_OF_EDGES][NUMBER_OF_LINES] = {0};
int edge_matrix_for_individual2[NUMBER_OF_TIME_INTERVALS][NUMBER_OF_EDGES][NUMBER_OF_LINES] = {0};
int edges_for_f3[NUMBER_OF_EDGES] = {0};

String_Rep population[POPULATION_SIZE] = {0};
String_Rep population_backup[POPULATION_SIZE] = {0};
...
double compute_fitness(String_Rep solution, int solution_index)
{
    ...
    memset(edges_for_f3, 0, sizeof(edges_for_f3));
...
{
    memset(edge_matrix, -1, sizeof(edge_matrix));
}

void reset_edge_matrix_for_individual(int individual_index)
{
    memset(edge_matrix[individual_index], -1, sizeof(edge_matrix[individual_index]));
}
...
int init_test(const char* edge_file_path, const char* demand_file_path, const char* path_file_path) {
    ...
    // bunlar dynamik olarak allocate bölümüne de eklenecek
        if (demands != NULL) {
        for (int i = 0; i < number_of_demand; i++) {
            free(demands[i].EST);
            free(demands[i].LAT);
    ...
    memset(all_alternative_paths_for_ODs, 0, sizeof(all_alternative_paths_for_ODs));
    memset(alternative_paths_for_ODs, 0, sizeof(alternative_paths_for_ODs));
    memset(population, 0, sizeof(population));
    memset(population_backup, 0, sizeof(population_backup));

    // şuraya kadar dynamik olarak allocate edilen koda eklenecek
